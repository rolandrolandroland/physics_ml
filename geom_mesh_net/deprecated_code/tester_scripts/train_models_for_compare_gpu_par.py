...
if __name__ == '__main__':
    ...
    # Determine the best device (Uses M1 GPU if available)
    device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
    print(f"Accelerating training using device: {device}")

    # Parallelize Data Loading using num_workers
    dataloader = DataLoader(
        dataset,
        batch_size=1,
        shuffle=True,
        collate_fn=dl.point_cloud_collate,
        num_workers=4,  # <-- NEW: Uses 4 CPU cores to process data simultaneously
        pin_memory=False  # <-- FIXED: MPS doesn't support pin_memory yet, this removes the warning
    )
    epochs = 1000
    ...
    for model_name, model in models.items():
        ...
        for epoch in range(epochs):
            ...
            for batch in dataloader:
                ...
                # Send inputs and targets to the M1 GPU
                xyz_inputs = torch.stack([x_flat, y_flat, z_flat], dim=1).float().to(device)
                targets = full_upp_probs.flatten().unsqueeze(1).float().to(device)

                # FORMAT INPUTS DEPENDING ON WHICH MODEL IS RUNNING
                if "Spatial_Stats" in model_name:
                    n_points = xyz_inputs.shape[0]
                    barcode_expanded = barcode.repeat(n_points, 1).float().to(device)
